      ...
      <AdminPageHeader
        ...
        actions={
          <Link
            ...
            className="inline-flex items-center border border-[var(--border)] px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-[var(--fg-subtle)] transition-colors hover:border-[var(--fg)] hover:text-[var(--fg)]"
          >
            ← 一覧へ
          </Link>
        }
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-6">
          <AdminSurface>
            <div className="border-b border-[var(--border)] px-6 py-4">
              <div className="flex items-center gap-3">
                ...
                <p className="label-en text-[var(--fg-subtle)]">Application Info</p>
              </div>
            </div>
          ...
          <AdminSurface className="p-6">
            <p className="label-en text-[var(--fg-subtle)]">Message</p>
            <h2 className="mt-1 font-bold text-[var(--fg)]">志望動機・メッセージ</h2>
            <div className="mt-4 border border-[var(--border)] bg-[var(--card-off)] p-5">
              <p className="whitespace-pre-wrap text-sm leading-8 text-[var(--fg)]">
                {app.message || "（未記入）"}
              </p>
            </div>
          </AdminSurface>
          ...
          {app.resumeUrl && (
            <AdminSurface className="p-6">
              <p className="label-en text-[var(--fg-subtle)]">Resume</p>
              <h2 className="mt-1 font-bold text-[var(--fg)]">添付履歴書</h2>
              <div className="mt-4 flex items-center gap-4">
                <span className="min-w-0 flex-1 truncate text-sm text-[var(--fg-subtle)]">
                ...
                <a
                  ...
                  className="shrink-0 bg-[var(--cta)] px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-[var(--cta-text)] transition-opacity hover:opacity-70"
                >
                  ダウンロード
                </a>
              </div>
        ...
        <div className="space-y-6">
          ...
          {/* ステータス更新 */}
          <AdminSurface className="p-6">
            <p className="label-en text-[var(--fg-subtle)]">Status</p>
            <h2 className="mt-1 font-bold text-[var(--fg)]">ステータス更新</h2>
                ...
                >
                  <button
                    ...
                    className={[
                      ...
                      app.status === value
                        ? "border-[var(--fg)] bg-[var(--cta)] text-[var(--cta-text)]"
                        : "border-[var(--border)] text-[var(--fg-subtle)] hover:border-[var(--fg)] hover:text-[var(--fg)]",
                    ].join(" ")}
                  >
                    {label}
                    {app.status === value && " ✓"}
          ...
          {/* 削除 */}
          <AdminSurface className="p-6">
            <p className="label-en text-[var(--fg-subtle)]">Danger Zone</p>
            <h2 className="mt-1 font-bold text-[var(--fg)]">応募データの削除</h2>
            <p className="mt-2 text-xs leading-6 text-[var(--fg-subtle)]">
              削除すると元に戻せません。
            </p>
